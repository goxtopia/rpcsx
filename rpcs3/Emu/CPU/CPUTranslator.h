...
#ifdef ARCH_ARM64
	// All ARMv8 CPUs have FMA.
	bool m_use_fma = true;

	// This flag is currently also used to parallelize SPU verification.
	bool m_use_avx = true;

	// Optional AArch64 integer matrix/vector extensions.
	bool m_use_dotprod = false;
	bool m_use_i8mm = false;

	// Avoid LLVM register-allocation failures seen with direct TBL2/TBX2.
	// The helper still lowers two-table operations to safe TBL1/TBX1 pairs.
	bool m_use_tbl2 = false;
...
public:
	...
	value_t<u8[16]> tbl(T1 a, T2 b)
	{
		...
		if (auto c = llvm::dyn_cast<llvm::Constant>(index))
		{
			...
			if (cv)
			{
				for (u32 i = 0; i < 16; i++)
				{
					const u64 index_value = cv->getElementAsInteger(i);
					mask._u8[i] = index_value < 16 ? static_cast<u8>(index_value) : static_cast<u8>(16);
				}
			}
	...
	value_t<u8[16]> tbl2(T1 a, T2 b, T3 indices)
	{
		...
		const auto first = m_ir->CreateCall(get_intrinsic<u8[16]>(llvm::Intrinsic::aarch64_neon_tbl1), {data0, index});
		const auto second_index = m_ir->CreateSub(index, llvm::ConstantInt::get(get_type<u8[16]>(), 16));
		const auto second = m_ir->CreateCall(get_intrinsic<u8[16]>(llvm::Intrinsic::aarch64_neon_tbl1), {data1, second_index});
		result.value = m_ir->CreateOr(first, second);
		return result;
	}
	...
	{
		value_t<u8[16]> result;
		result.value = m_ir->CreateCall(get_intrinsic<u8[16]>(llvm::Intrinsic::aarch64_neon_tbx1), {fallback.eval(m_ir), a.eval(m_ir), indices.eval(m_ir)});
		return result;
	}
	...
	{
		value_t<u8[16]> result;
		const auto fallback_value = fallback.eval(m_ir);
		const auto data0 = a.eval(m_ir);
		const auto data1 = b.eval(m_ir);
		const auto index = indices.eval(m_ir);
		...
		if (m_use_tbl2)
		{
			result.value = m_ir->CreateCall(get_intrinsic<u8[16]>(llvm::Intrinsic::aarch64_neon_tbx2), {fallback_value, data0, data1, index});
			return result;
		}

		const auto first = m_ir->CreateCall(get_intrinsic<u8[16]>(llvm::Intrinsic::aarch64_neon_tbx1), {fallback_value, data0, index});
		const auto second_index = m_ir->CreateSub(index, llvm::ConstantInt::get(get_type<u8[16]>(), 16));
		result.value = m_ir->CreateCall(get_intrinsic<u8[16]>(llvm::Intrinsic::aarch64_neon_tbx1), {first, data1, second_index});
		return result;
	}
#endif
	...
	{
		return value->getType() == fixed_type ? value : m_ir->CreateExtractVector(fixed_type, value, m_ir->getInt64(0));
	}
	...
	value_t<T> sve_mull(llvm::Intrinsic::ID id, T1 a, T2 b)
	{
		...
		result.value = from_sve_vector(m_ir->CreateIntrinsic(id, {scalable_type}, {data0, data1}), fixed_type);
		return result;
	}
	...
	value_t<T> sve_mlal(llvm::Intrinsic::ID id, T0 acc, T1 a, T2 b)
	{
		...
		result.value = from_sve_vector(m_ir->CreateIntrinsic(id, {scalable_type}, {data0, data1, data2}), fixed_type);
		return result;
	}

	template <typename T1, typename T2>
	value_t<s32[4]> sve_smullb(T1 a, T2 b) { return sve_mull<s32[4]>(llvm::Intrinsic::aarch64_sve_smullb, a, b); }
	template <typename T1, typename T2>
	value_t<s32[4]> sve_smullt(T1 a, T2 b) { return sve_mull<s32[4]>(llvm::Intrinsic::aarch64_sve_smullt, a, b); }
	template <typename T1, typename T2>
	value_t<u32[4]> sve_umullb(T1 a, T2 b) { return sve_mull<u32[4]>(llvm::Intrinsic::aarch64_sve_umullb, a, b); }
	template <typename T1, typename T2>
	value_t<u32[4]> sve_umullt(T1 a, T2 b) { return sve_mull<u32[4]>(llvm::Intrinsic::aarch64_sve_umullt, a, b); }
	template <typename T0, typename T1, typename T2>
	value_t<s32[4]> sve_smlalb(T0 acc, T1 a, T2 b) { return sve_mlal<s32[4]>(llvm::Intrinsic::aarch64_sve_smlalb, acc, a, b); }
	template <typename T0, typename T1, typename T2>
	value_t<s32[4]> sve_smlalt(T0 acc, T1 a, T2 b) { return sve_mlal<s32[4]>(llvm::Intrinsic::aarch64_sve_smlalt, acc, a, b); }
	template <typename T0, typename T1, typename T2>
	value_t<u32[4]> sve_umlalt(T0 acc, T1 a, T2 b) { return sve_mlal<u32[4]>(llvm::Intrinsic::aarch64_sve_umlalt, acc, a, b); }

	template <typename T1, typename T2, typename T3>
	value_t<u32[4]> udot(T1 a, T2 b, T3 c)
	{
		value_t<u32[4]> result;
		result.value = m_ir->CreateCall(get_intrinsic<u32[4], u8[16]>(llvm::Intrinsic::aarch64_neon_udot), {a.eval(m_ir), b.eval(m_ir), c.eval(m_ir)});
		return result;
	}
	...
	{
		value_t<u32[4]> result;
		result.value = m_ir->CreateCall(get_intrinsic<u32[4], u8[16]>(llvm::Intrinsic::aarch64_neon_sdot), {a.eval(m_ir), b.eval(m_ir), c.eval(m_ir)});
		return result;
	}
	...
	{
		value_t<u32[4]> result;
		result.value = m_ir->CreateCall(get_intrinsic<u32[4], u8[16]>(llvm::Intrinsic::aarch64_neon_ummla), {a.eval(m_ir), b.eval(m_ir), c.eval(m_ir)});
		return result;
	}
	...
	{
		value_t<u32[4]> result;
		result.value = m_ir->CreateCall(get_intrinsic<u32[4], u8[16]>(llvm::Intrinsic::aarch64_neon_smmla), {a.eval(m_ir), b.eval(m_ir), c.eval(m_ir)});
		return result;
	}
	...
	auto addp(T1 a, T2 b)
	{
		using vector_type = typename is_llvm_expr<T1>::type;
		value_t<vector_type> result;
		result.value = m_ir->CreateCall(get_intrinsic<vector_type>(llvm::Intrinsic::aarch64_neon_addp), {a.eval(m_ir), b.eval(m_ir)});
		return result;
	}
